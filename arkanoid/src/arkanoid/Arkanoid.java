...
public class Arkanoid extends JFrame implements KeyListener {
	...
	/* VARIABLE QUE SE INCREMENTA CADA VEZ QUE SE ROMPE UN LADRILLO */
	int numBricks=0;
	...
	HashMap <Color, Integer> puntuaciones = new HashMap<>();
	...
	class ScoreBoard {
		...
		void increaseScore(Color color) {
			
			score+=puntuaciones.get(color);
			
			if (numBricks == (COUNT_BLOCKS_X * COUNT_BLOCKS_Y)) {
				win = true;
	...
	class Brick extends Rectangle {
		...
		Brick(double x, double y, int color) {
			this.x = x;
			this.y = y;
			switch(color) {
	...
	void testCollision(Brick mBrick, Ball mBall, ScoreBoard scoreboard) {
		...
		scoreboard.increaseScore(mBrick.getColor());
	...
	void initializeBricks(List<Brick> bricks) {
		...
		puntuaciones.put(Color.RED, 1);
		puntuaciones.put(Color.GREEN, 2);
		puntuaciones.put(Color.YELLOW, 5);
		puntuaciones.put(Color.BLUE, 7);
